...
const Operacion = () => {
    ...
    const formSubmit = (data) => {
        actualizarOperacion(data);
    }

    return (
    <div>
        <Header titulo="Entrada de productos">ID Operacion: {operacion.id}</Header>
        <section className="d-flex justify-content-between mb-3">
            ...
            <div className="d-flex gap-1">
                <button onClick={() => setOpenModal(true)} className="btn btn-success">Guardar entrada</button>
                <button className="btn btn-primary">Autorizar entrada</button>
            </div>
        </section>
            ...
            <Modal.Body>
                <form onSubmit={handleSubmit(formSubmit)}>
                    <div className="mb-3">
                        ...
                        <input className="form-control" placeholder="Ingresa tu nombre" type="text" {...register("operador",{required: true})} />
                    </div>
                    <div className="mb-3 d-flex gap-1">
                        <div className="flex-grow-1">
                            <label className={`form-label ${errors.proveedor?.type === "required" && "text-danger fw-bold"}`}>Proveedor *</label>
                            <input className="form-control" placeholder="Nombre del proveedor" type="text" {...register("proveedor",{required: true})} />
                        </div>
                        <div className="flex-grow-1">
                            <label className={`form-label ${errors.empresa?.type === "required" && "text-danger fw-bold"}`}>Empresa *</label>
                            <input className="form-control" placeholder="Empresa que provee" type="text" {...register("empresa",{required: true})} />
                        </div>
                    </div>
                    <div className="mb-3 d-flex gap-1">
                        <div className="flex-grow-1">
                            <label className={`form-label ${errors.no_documento?.type === "required" && "text-danger fw-bold"}`}>ID del documento *</label>
                            <input className="form-control" placeholder="Número del documento" type="text" {...register("no_documento",{required: true})} />
                        </div>
                        <div className="flex-grow-1">
                            <label className={`form-label ${errors.documento?.type === "required" && "text-danger fw-bold"}`}>Documento *</label>
                            <input className="form-control" placeholder="Tipo de documento" type="text" {...register("documento",{required: true})} />
                        </div>
                    </div>
        ...
        <article className="row">
            <section className="col-3 border-end" style={{maxHeight: "60vh"}}>
                <table className="table">
                    ...
                    {filtrado.length > 0 ? filtrado.map(producto => (
                        <tr key={producto.id}>
                            ...
                            <td><button onClick={() => agregarProducto({id_inventario:producto.id})}>Add</button></td>
                        </tr>
                    )): (
                        <tr>
                    ...
                    <tbody>
                        {productosMovimiento.length > 0 ?  productosMovimiento.map(producto => (
                            <tr key={producto.id}>
                                ...
                                <td><button onClick={() => quitarProducto({id:producto.id})} >Quitar</button></td>
                            </tr>
                        )) : (
                            <tr>
